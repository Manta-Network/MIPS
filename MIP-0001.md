...
* *Shielded Address*. A shielded address is the address that the receiver need to send to the 
  ...
  (see `MantaAssetShieldedAddress` in `manta-types/manta-asset`):
    ...
    - `k`: public information `k` (will be explained later) (TODO(ask zz): why is the public?)
...
* *UTXO*. MantaPay uses UTXOs rather than account for the private coins. Both *Mint* and *PrivateTranfer* appends a new UTXO on-chain.
* *Ledger State*. 
