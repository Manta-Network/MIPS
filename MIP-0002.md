...
`Mint`(`LS`, `tx`<sub>mint</sub>, `sig`(`Alice`, `tx`<sub>mint</sub>)) &rarr; 
  ...
  - applies the sharding function `H`<sub>shard</sub> (here we use Blake2s, with 1 byte output) to `utxo` to compute its shard index: `i`:=`sh`(`utxo`) and adds `utxo` to the accumulator shard `acc`<sub>i</sub>. 
